...
class InMemoryLeadRepositoryTest {
  ...
  void shouldReturnSingleLeadInListWhenAddUniqueLeadToEmptyRepository() {
    ...
    assertThat(repository.findAll()).hasSize(1).containsExactly(firstLead);

    assertThat(repository.findById(firstLead.getId())).isPresent().get().isEqualTo(firstLead);
  }
  ...
  void shouldRejectDuplicateLeadAndKeepSizeOneWhenAddLeadWithSameIdAsExisting() {
    ...
    Lead lead1 =
        new Lead(sharedId, createContact("john1@example.com", "+71234567899"), "Corp1", "NEW");
    Lead lead2 =
        new Lead(
            sharedId, createContact("john2@example.com", "+71234567890"), "Corp2", "QUALIFIED");
    ...
    assertThat(repository.findAll()).hasSize(1).extracting(Lead::getId).containsExactly(sharedId);
  }
