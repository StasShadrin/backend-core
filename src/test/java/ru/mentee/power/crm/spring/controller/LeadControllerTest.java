...
class LeadControllerTest {
  ...
  void shouldShowEditFormForExistingLead() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/leads/{id}/edit", validId))
  ...
  void shouldFilterBySearchTerm() throws Exception {
    ...
    Lead expectedLead =
        LeadBuilder.builder()
            ...
            .status(LeadStatus.NEW)
            .build();
    when(leadService.findLeads((searchTerm), (null))).thenReturn(List.of(expectedLead));

    // When & Then
    mockMvc
        .perform(get("/leads").param("search", searchTerm))
        .andExpect(status().isOk())
  ...
  void shouldFilterByStatus() throws Exception {
    ...
    when(leadService.findLeads((null), (status))).thenReturn(expectedLeads);

    // When & Then
    mockMvc
        .perform(get("/leads").param("status", status.name()))
        .andExpect(status().isOk())
        .andExpect(view().name("leads/list"))
  ...
  void shouldReturnAllLeadsWhenNoParameters() throws Exception {
    ...
    when(leadService.findLeads((null), null)).thenReturn(allLeads);
  ...
  void shouldCombineSearchAndStatusFilters() throws Exception {
    ...
    Lead expectedLead =
        LeadBuilder.builder()
            ...
            .status(status)
            .build();
    when(leadService.findLeads((searchTerm), (status))).thenReturn(List.of(expectedLead));

    // When & Then
    mockMvc
        .perform(get("/leads").param("search", searchTerm).param("status", status.name()))
        .andExpect(status().isOk())
        .andExpect(view().name("leads/list"))
        .andExpect(model().attribute("leads", List.of(expectedLead)))
        .andExpect(model().attribute("search", searchTerm))
