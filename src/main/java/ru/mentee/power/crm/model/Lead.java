...
public record Lead(
    ...
    @NotBlank(message = "{lead.email.notblank}")
        ...
        String email,
    @NotBlank(message = "{lead.phone.notblank}") @Size(max = 20, message = "{lead.phone.size}")
        String phone,
    @Size(min = 2, max = 100, message = "{lead.company.size}") String company,
    @NotNull(message = "{lead.status.notnull}") LeadStatus status) {}
